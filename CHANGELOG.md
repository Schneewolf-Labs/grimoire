...
### Changed
- Reference-model losses (DPO, IPO, KTO) now run the frozen reference forward BEFORE the policy forward. The reference pass carries no gradients, so computing it first means its activations never coexist with the policy's retained autograd graph — peak memory drops by roughly the reference pass's footprint. Loss values are unchanged (the two passes are independent).
