...
class GrimoireTrainer:
    ...
    def train(self):
        ...
        for epoch in range(starting_epoch, config.num_epochs):
            ...
            for step, batch in enumerate(active_dataloader):
                loss, metrics = self._train_batch(batch)
    ...
    def _train_batch(self, batch):
        """Run one optimization step for a single batch and return (loss, metrics).

        Default behavior: a single forward/backward/step under the gradient
        accumulation context. Subclasses (e.g. GRPOTrainer) override this to
        implement stateful/online steps such as rollout generation.
        """
        config = self.config
