...
from grimoire.data.prompt import PromptCollator, tokenize_prompt
...
class TestPromptCollator:
    ...
    def test_only_has_input_ids_and_attention_mask(self):
        """Prompt collator should not produce labels."""
        collator = PromptCollator(pad_token_id=0)
        features = [{"input_ids": [1, 2], "attention_mask": [1, 1]}]
        batch = collator(features)
...
class TestTokenizePrompt:
    ...
    def test_produces_correct_keys(self, mock_tokenizer):
        ...
        assert "input_ids" in result
        assert "attention_mask" in result
        assert "labels" not in result  # prompts don't have labels

    def test_tokenizes_prompt_field(self, mock_tokenizer):
        example = {"prompt": "AB"}
        result = tokenize_prompt(example, mock_tokenizer)
    ...
    def test_custom_prompt_field(self, mock_tokenizer):
        example = {"question": "XY"}
        result = tokenize_prompt(example, mock_tokenizer, prompt_field="question")

        assert result["input_ids"] == [ord("X"), ord("Y")]
