...
class TestGRPOTrainerSmoke:
    ...
    def test_generation_batch_size_runs(self):
        torch.manual_seed(0)
        tmpdir = tempfile.mkdtemp()
        try:
            trainer = GRPOTrainer(
                model=TinyGenerativeLM(),
                tokenizer=GenTokenizer(),
                config=_config(tmpdir, generation_batch_size=1),
                loss_fn=GRPOLoss(beta=0.0),
                reward_fn=_length_reward,
                train_dataset=make_prompt_dataset(n=8),
            )
...
class TestGRPOTrainerUnits:
    ...
    def test_reward_count_must_match_completion_count(self):
        tmpdir = tempfile.mkdtemp()
        try:
            t = self._trainer(tmpdir)
            t.reward_fn = lambda prompts, completions, **cols: [1.0]
